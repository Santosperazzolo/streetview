...
app.post("/api/projects/:projectId/video", verifyToken, isAdmin, upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    // Automáticamente extraer frames SINCRONICAMENTE si hay puntos GPS
    ...
    if (gpsPoints.length > 0) {
      ...
      try {
        ...
        if (frames.length === 0) {
          ...
          return res.json({ ok: true, videoUrl, framesExtracted: 0, message: "Video subido pero sin frames extraídos" });
        }
        ...
        res.json({ ok: true, videoUrl, framesExtracted: frames.length, message: `${frames.length} frames extraídos` });
      } catch (frameError) {
